	...
	public class IndexModel : PageModel
    {
        ...
        public async Task OnGetAsync()
        {
            ...
            DocsAttachments = docsAttachments
                .Where(x => x.DocumentTrackings.FirstOrDefault(x => x.ReviewerId == account.Id)?.ReviewerStatus == ReviewerStatus.Approved && !x.DocumentTrackings.Any(g => g.ReviewerStatus == ReviewerStatus.Completed) || x.DocumentTrackings.FirstOrDefault(x => x.ReviewerId == account.Id)?.ReviewerStatus == ReviewerStatus.Passed && !x.DocumentTrackings.Any(g => g.ReviewerStatus == ReviewerStatus.Completed) || x.DocumentTrackings.FirstOrDefault(x => x.ReviewerId == account.Id)?.ReviewerStatus == ReviewerStatus.Reviewed  && x.DocumentTrackings.Any(r => r.ReviewerStatus == ReviewerStatus.Approved) && !x.DocumentTrackings.Any(r => r.ReviewerStatus == ReviewerStatus.Completed) || x.DocumentAttachment.DocumentType == DocumentType.WalkIn && x.DocumentTrackings.Any(e => e.ReviewerId != account.Id) && designationName == firstDesingationName && !x.DocumentTrackings.Any(t => t.ReviewerStatus == ReviewerStatus.Approved))
                .ToList();
