...
namespace WebTrackED_CHED_MIMAROPA.Pages.Application.Document.Pending
{
    ...
    public class IndexModel : BasePageModel<DocumentAttachment, DocumentAttachment>
    {
        ...
        public async Task OnGetAsync() 
        {
            ...
            this.docAttachments = docAttachments.Where(x => x.SenderAccount.Id == user.Id && x.DocumentTrackings.Count() <= 1 || x.DocumentTrackings.Count() > 1 && !x.DocumentTrackings.Any(x => x.ReviewerStatus == ReviewerStatus.OnReview)).ToList();
        }
